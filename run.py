...
import config
from data_loader import load_dummy, load_hotpotqa
from evaluate import evaluate, format_results
from retrievers import get_retriever


def parse_args():
    parser = argparse.ArgumentParser(description="Evaluate RAG retrievers on HotpotQA")
    parser.add_argument(
        "--retrievers",
        nargs="+",
    ...
    parser.add_argument(
        ...
        help="local folder for HotpotQA; downloads from HF only if missing",
    )
    parser.add_argument(
        "--chunk",
...
def main():
    ...
    if args.dummy:
        ...
        corpus, queries, qrels = load_hotpotqa(
            split=args.split,
    ...
    for spec in args.retrievers:
        retriever = get_retriever(spec, **kwargs)
        print(f"\nindexing {spec} ...")
        retriever.index(corpus)
        print(f"evaluating {spec} ...")
        all_results[spec] = evaluate(retriever, queries, qrels, k_values=k_values)
